...
function selecionar(elemento, classe) {
  ...
  if (selecionado !== null) {
    let classeIgual =
      classe === selecionado.parentElement.parentElement.classList.item(1);

    // Verifica se são o mesmo elemente e se são da mesma classe
    if (
      elemento.querySelector("h3").innerHTML !==
        selecionado.querySelector("h3").innerHTML &&
      classeIgual
    ) {
...
function formatoBotaoComprar() {
  ...
  if (totalSelecionado === 0) {
    botaoFechaPedido.querySelector("p").innerHTML =
      "<p>Selecione os 3 itens<br />para fechar o pedido</p>";
  } else if (totalSelecionado === 1) {
    botaoFechaPedido.querySelector("p").innerHTML =
      "<p>Selecione mais 2 itens<br />para fechar o pedido</p>";
  } else if (totalSelecionado === 2) {
    botaoFechaPedido.querySelector("p").innerHTML =
      "<p>Selecione mais 1 item<br />para fechar o pedido</p>";
...
// Cancela pedido e voltar para página anterior
function cancelarPedido() {
  document.querySelector(".confirma-pedido").classList.add("hidden");
}

// Confirma pedido e enviar por WhatsApp
